describe("Time series transformations card", () => {
  ...
  it("Transformations dropdown is visible and shows correct options", () => {
    ...
    cy.contains('[role="option"]', 'Original').should('exist');
    cy.contains('[role="option"]', 'First transformation').should('exist');
    cy.contains('[role="option"]', 'Second transformation').should('exist');
  ...
  it("Scales dropdown is visible and shows correct options", () => {
    ...
    cy.contains('[role="option"]', 'Exact').should('exist');
    cy.contains('[role="option"]', 'From 0 to 1').should('exist');
    cy.contains('[role="option"]', 'From -1 to 1').should('exist');
    cy.get('body').type('{esc}');
  });
